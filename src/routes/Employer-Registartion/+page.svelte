...
const res=await fetch(`https://nominatim.openstreetmap.org/search?q=${form.location}&format=json&addressdetails=1&limit=5`);
...
form.zipcode=addr.postcode||"";

showSuggestions=false;

}

/* ---------------- DISTRICT TALUKA DATA ---------------- */
...
"VADODARA":["Dabhoi","Desar","Karajan","Padra","Savli","Sinor","Vadodara","Vaghodia"],
...
<div class="option" on:click={()=>selectLocation(item)}>
{item.display_name}
</div>
...
<div class="field">
<label>City</label>
<input bind:value={form.city} readonly>
</div>

<div class="field">
<label>State</label>
<input bind:value={form.state} readonly>
</div>

<div class="field">
<label>Country</label>
<input bind:value={form.country} readonly>
</div>
...
<select bind:value={form.district} on:change={handleDistrict}>
...
<select bind:value={form.taluka} disabled={!form.district}>
